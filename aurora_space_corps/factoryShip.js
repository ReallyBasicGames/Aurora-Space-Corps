class FactoryShip extends Ship {
    constructor(x, y, team, image) {
        ...
        this.health = 10000; // how much health this has. Standard bullets deal 1 damage
    ...
    buy(s) {
        ...
        var teamNum = this.team;

        switch (s) {
            case "small":
                if (this.power >= this.pricesShips[0]) {
                    this.power -= this.pricesShips[0];
                    shipHandler.makeShip("chaos", this.team, this.body.position.x, this.body.position.y, false, this.extraDamage, this.extraHealth, chaosFighterImg, chaosLightsImg[teamNum]);
            ...
            case "medium":
                if (this.power >= this.pricesShips[1]) {
                    ...
                    shipHandler.makeShip("eclipse", this.team, this.body.position.x, this.body.position.y, false, this.extraDamage, this.extraHealth, eclipseFighterImg, eclipseLightsImg[teamNum]);
                    this.lastSpent = millis();
                    this.shipsBuilt[1]++;
                    if (this.isPlayerControlled) shipsBuilt++;
                }
            ...
            case "large":
                if (this.power >= this.pricesShips[2]) {
                    ...
                    shipHandler.makeShip("nebula", this.team, this.body.position.x, this.body.position.y, false, this.extraDamage, this.extraHealth, nebulaFighterImg, nebulaLightsImg[teamNum]);
                    this.lastSpent = millis();
                    this.shipsBuilt[2]++;
                    if (this.isPlayerControlled) shipsBuilt++;
                }
